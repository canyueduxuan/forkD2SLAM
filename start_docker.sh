...
DOCKERIMAGE="hkustswarm/d2slam:x86"
if [ $# -eq 0 ]; then
...
if [ ${START_OPTION} == 1 ]; then
  ...
  docker run -it --rm --runtime=nvidia --gpus all  --net=host -v ${CURRENT_DIR}:${SWARM_WS}/src/D2SLAM \
    -v /dev/:/dev/   --privileged -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix  --name="d2slam_container" ${DOCKERIMAGE} /bin/bash 
else
  echo "Start docker container for image transportation only"
  docker run -it --rm --runtime=nvidia --gpus all  --net=host -v /dev/:/dev/ \
    --privileged -e DISPLAY    -v /tmp/.X11-unix:/tmp/.X11-unix --name="d2slam_container"  ${DOCKERIMAGE} /bin/bash
fi
